...
#define CALLSTACK_CHUNK_SIZE                                                   \
  32 // Number of frame slots in each CallStack chunk.
#define MAX_SYMBOL_NAME_LENGTH                                                 \
  256 // Maximum symbol name length that we will allow. Longer names will be
      // truncated.
...
//
//    IMPORTANT NOTE: This class as originally written makes two fatal
//    assumptions: First: That the application will never load modules (call
//    LoadLibrary) during the
//           lifetime of the app.
...
//    memory allocations whose call stacks goes through dynamically loaded
//    modules will be incomplete. This is manifested by stack frames that cannot
//    be resolved when the stack is 'dumped' because the binaries have been
//    unloaded from the process. To rectify this situation, it is up to the
//    caller to resolve or format the call stacks into human readable form,
//    BEFORE the callers process unloads any modules. That is done by calling
//    VisualLeakDetector::ResolveCallstacks, which can be called from external
//    code by the exported VLDResolveCallstacks function. When this happens, the
//    call stacks are formatted, and then cached for later dumping. This has
//    performance penalties, as the current implementation saves all info to
//    heap memory, and it is wasteful, as some of the 'converted' memory is not
//    a true leak, but will get properly de-allocated at a later time. However
//    there is no other way to work around the fact that the call stacks can
//    only get formatted when the binary is loaded in the process.
//
class CallStack {
public:
  CallStack();
  ...
  // Formats the stack frame into a human readable format, and saves it for
  // later retrieval.
...
#define CALLSTACK_STATUS_INCOMPLETE                                            \
  0x1 //   If set, the stack trace stored in this CallStack appears to be
      //   incomplete.
#define CALLSTACK_STATUS_STARTUPCRT                                            \
  0x2 //   If set, the stack trace is startup CRT.
#define CALLSTACK_STATUS_NOTSTARTUPCRT                                         \
  0x4 //   If set, the stack trace is not startup CRT.

  // The chunk list is made of a linked list of Chunks.
  struct chunk_t {
    chunk_t *next; // Pointer to the next chunk in the chunk list.
    UINT_PTR
        frames[CALLSTACK_CHUNK_SIZE]; // Pushed frames (program counter
                                      // addresses) are stored in this array.
  };

  // Private data.
  UINT32 m_capacity;          // Current capacity limit (in frames)
  UINT32 m_size;              // Current size (in frames)
  CallStack::chunk_t m_store; // Pointer to the underlying data store (i.e. head
                              // of the chunk list)
  CallStack::chunk_t
      *m_topChunk;   // Pointer to the chunk at the top of the stack
  ...
  LPCWSTR getFunctionName(SIZE_T programCounter, DWORD64 &displacement64,
                          ...
                          CriticalSectionLocker<DbgHelp> &locker) const;
  DWORD resolveFunction(SIZE_T programCounter, IMAGEHLP_LINEW64 *sourceInfo,
                        DWORD displacement, LPCWSTR functionName,
                        LPWSTR stack_line, DWORD stackLineSize) const;
...
public:
  FastCallStack() : m_hashValue(0) {}
  virtual VOID getStackTrace(UINT32 maxdepth, const context_t &context);
  virtual DWORD getHashValue() const { return m_hashValue; }
