...
bool endWith(const LPCWSTR filename, size_t len, wchar_t const (&substr)[N]) {
  size_t count = N - 1;
  return ((len >= count) &&
          wcsncmp(filename + len - count, substr, count) == 0);
}
...
LPCWSTR
CallStack::getFunctionName(SIZE_T programCounter, DWORD64 &displacement64,
  ...
  if (g_DbgHelp.SymFromAddrW(g_currentProcess, programCounter, &displacement64,
                             functionInfo, locker)) {
    functionName = functionInfo->Name;
  } else {
// GetFormattedMessage( GetLastError() );
#ifdef USE_FMT
    const auto &address = fmt::format(L"" ADDRESSCPPFORMAT, programCounter);
    strncpy((char *)functionInfo->Name, (char *)address.c_str(),
...
DWORD CallStack::resolveFunction(SIZE_T programCounter,
                                 IMAGEHLP_LINEW64 *sourceInfo,
  ...
  if (hCallingModule && GetModuleFileName(hCallingModule, callingModuleName,
                                          _countof(callingModuleName)) > 0) {
  ...
  if (sourceInfo) {
    if (displacement == 0) {
      w = fmt::format(L"    {} ({}): {}!{}()\n", sourceInfo->FileName,
                      sourceInfo->LineNumber, moduleName, functionName);
    } else {
      w = fmt::format(L"    {} ({}): {}!{}() + 0x{:X} bytes\n",
  ...
  strncpy((char *)stack_line, (char *)w.c_str(),
          stackLineSize * sizeof(wchar_t));
  return NumChars;
#else
  ...
  if (sourceInfo) {
    if (displacement == 0) {
      ...
      w.write(L"    {} ({}): {}!{}() + 0x{:X} bytes\n", sourceInfo->FileName,
              sourceInfo->LineNumber, moduleName, functionName, displacement);
    }
  } else {
    if (displacement == 0) {
      w.write(L"    {}!{}()\n", moduleName, functionName);
...
// isCrtStartupAlloc - Determines whether the memory leak was generated from crt
// startup code. This is not an actual memory leaks as it is freed by crt after
// the VLD object has been destroyed.
//
//  Return Value:
//
...
bool CallStack::isCrtStartupAlloc() {
  ...
  for (UINT32 frame = 0; frame < m_size; frame++) {
    ...
    LPCWSTR functionName = getFunctionName(
        programCounter, displacement64, (SYMBOL_INFO *)&symbolBuffer, locker);
...
int CallStack::resolve(BOOL showInternalFrames) {
  ...
  if (m_status & CALLSTACK_STATUS_INCOMPLETE) {
    ...
    // more reliable.
    Report(L"    HINT: The following call stack may be incomplete. Setting "
           L"\"StackWalkMethod\"\n"
           L"      in the vld.ini file to \"safe\" instead of \"fast\" may "
           L"result in a more\n"
  ...
  bool skipStartupLeaks =
      !!(g_vld.GetOptions() & VLD_OPT_SKIP_CRTSTARTUP_LEAKS);
  ...
  for (UINT32 frame = 0; frame < m_size; frame++) {
    ...
    if (skipStartupLeaks) {
      if (!(m_status &
            (CALLSTACK_STATUS_STARTUPCRT | CALLSTACK_STATUS_NOTSTARTUPCRT))) {
        m_status |= isCrtStartupFunction(functionName);
      }
      if (m_status & CALLSTACK_STATUS_STARTUPCRT) {
        delete[] m_resolved;
    ...
    // It turns out that calls to SymGetLineFromAddrW64 may free the very memory
    // we are scrutinizing here in this method. If this is the case, m_Resolved
    // will be null after SymGetLineFromAddrW64 returns. When that happens there
    // is nothing we can do except crash.
    DWORD displacement = 0;
    DbgTrace(L"dbghelp32.dll %i: SymGetLineFromAddrW64\n",
             GetCurrentThreadId());
    BOOL foundline = g_DbgHelp.SymGetLineFromAddrW64(
        g_currentProcess, programCounter, &displacement, &sourceInfo, locker);
    ...
    NumChars = resolveFunction(programCounter, foundline ? &sourceInfo : NULL,
                               displacement, functionName, stack_line,
                               _countof(stack_line));
  ...
  while (count < maxdepth) {
    if (*framePointer < (UINT_PTR)framePointer) {
      if (*framePointer == NULL) {
        ...
        // Invalid frame pointer. Frame pointer addresses should
        always
    ...
    if (*framePointer & (sizeof(UINT_PTR *) - 1)) {
      ...
      // we've encountered a frame that was created by a module built
      with
...
#elif defined(_M_X64)
  UINT32 maxframes = std::min((UINT32)62, (UINT32)(maxdepth + 10));
  UINT_PTR *myFrames = new UINT_PTR[maxframes];
  ZeroMemory(myFrames, sizeof(UINT_PTR) * maxframes);
  ULONG BackTraceHash;
  maxframes = RtlCaptureStackBackTrace(
      0, maxframes, reinterpret_cast<PVOID *>(myFrames), &BackTraceHash);
...
VOID SafeCallStack::getStackTrace(UINT32 maxdepth, const context_t &context) {
  ...
  while (count < maxdepth) {
    ...
    if (!g_DbgHelp.StackWalk64(architecture, g_currentProcess, g_currentThread,
                               &frame, &currentContext, NULL,
