...
#define BLOCK_MAP_RESERVE                                                      \
  64 // This should strike a balance between memory use and a desire to minimize
     // heap hits.
#define HEAP_MAP_RESERVE                                                       \
  2 // Usually there won't be more than a few heaps in the process, so this
    // should be small.
#define MODULE_SET_RESERVE                                                     \
  16 // There are likely to be several modules loaded in the process.
...
HANDLE g_processHeap; // Handle to the process's heap (COM allocations come from
                      // here).
...
patchentry_t ldrLoadDllPatch[] = {"LdrLoadDll",
                                  NULL,
                                  VisualLeakDetector::_LdrLoadDll,
                                  "LdrGetDllHandle",
                                  NULL,
                                  VisualLeakDetector::_LdrGetDllHandle,
                                  "LdrGetProcedureAddress",
                                  NULL,
                                  VisualLeakDetector::_LdrGetProcedureAddress,
                                  "LdrLockLoaderLock",
                                  NULL,
                                  VisualLeakDetector::_LdrLockLoaderLock,
                                  "LdrUnlockLoaderLock",
                                  NULL,
                                  VisualLeakDetector::_LdrUnlockLoaderLock,
                                  NULL,
                                  NULL,
                                  NULL};
...
// We provide our own DllEntryPoint in order to capture the ReturnAddress of the
// function calling our DllEntryPoint. Then we patch this function namely
// ntdll.dll!LdrpCallInitRoutine or any equivalent NT loader function by calling
// our LdrpCallInitRoutine where we RefreshModules before calling the EntryPoint
// in order to hook all functions required to properly capture all _CRT_INIT
// memory allocations which include internal CRT startup memory allocations and
// all global and static initializers.
//
// In getLeaksCount(), reportLeaks() and resolveStacks(), we take extra measures
// to identify and exclude debug and release internal CRT allocations from
// reporting as real memory leaks.
//
// Global and static initializers *might* be reported as memory leaks based on
// the order being unintialised by _CRT_INIT.
...
PBYTE NtDllFindParamAddress(const PBYTE pAddress) {
  ...
  // for 64bit find => mov r8,... => parameters are moved to registers r8, edx,
  // rcx
  while (pAddress - --ptr < 0x20) {
#ifdef _WIN64
    if (((ptr[0] & 0x4D) >= 0x4C) && (ptr[1] == 0x8B) &&
        ((ptr[2] & 0xC7) == ptr[2])) {
#else
...
BOOL NtDllPatch(const PBYTE pReturnAddress, NTDLL_LDR_PATCH &NtDllPatch) {
  if (NtDllPatch.bState == FALSE) {
    ...
    NtDllPatch.nDetourSize =
        _countof(ptr) + nParamSize + _countof(mov) + _countof(jmp);
    NtDllPatch.pDetourAddress =
        NtDllFindDetourAddress(pReturnAddress, NtDllPatch.nDetourSize);

    if (NtDllPatch.pPatchAddress && NtDllPatch.pDetourAddress &&
        ((_countof(jmp) + _countof(call)) <= NtDllPatch.nPatchSize)) {
      memcpy(NtDllPatch.pBackup, NtDllPatch.pPatchAddress,
             NtDllPatch.nPatchSize);

      DWORD dwProtect = 0;
      if (VirtualProtect(NtDllPatch.pDetourAddress, NtDllPatch.nDetourSize,
        ...
        LPBYTE icall = NtDllPatch.pPatchAddress + nParamSize -
                       (3 /*instruction size*/ + sizeof(DWORD));
        if ((*(LPDWORD)icall & 0x000D8B4C) == 0x000D8B4C) {
          // From Windows 10 (1607) calls to the EntryPoint are dispatched
          // through
          // __guard_dispatch_icall_fptr. In such case correct the relative
          // address.
          DWORD fptr =
              *(LPDWORD)(icall + 3) + (3 /*instruction size*/ + sizeof(DWORD)) -
              (DWORD)(NtDllPatch.pDetourAddress - NtDllPatch.pPatchAddress);
          memcpy(NtDllPatch.pDetourAddress + nParamSize - sizeof(DWORD), &fptr,
                 sizeof(DWORD));

          // Additionally in such case the EntryPoint is held in another
          // register that was moved to rax. In such case identify the correct
          // register holding the EntryPoint
          reg = ((*(icall - 3) & 0xF1) == 0x41 ? 0x08 : 0x00) +
                (*(icall - 1) & 0x07);
        } else {
          reg = ((pCallAddress[0] & 0xF1) == 0x41 ? 0x08 : 0x00) +
                (pCallAddress[pReturnAddress - pCallAddress - 1] & 0x07);
        }
        ...
        // Copy original param instructions
        memcpy(&NtDllPatch.pDetourAddress[_countof(ptr)],
               NtDllPatch.pPatchAddress, nParamSize);
#endif
        // Move LdrpCallInitRoutine to eax/rax
        *(PSIZE_T)(&mov[2]) = (SIZE_T)LdrpCallInitRoutine;
        memcpy(&NtDllPatch.pDetourAddress[_countof(ptr) + nParamSize], &mov,
               _countof(mov));

        // Jump to original function
        *(DWORD *)(&jmp[1]) =
            (DWORD)(pReturnAddress - _countof(call) -
                    (NtDllPatch.pDetourAddress + NtDllPatch.nDetourSize));
        memcpy(&NtDllPatch
                    .pDetourAddress[_countof(ptr) + nParamSize + _countof(mov)],
               &jmp, _countof(jmp));

        VirtualProtect(NtDllPatch.pDetourAddress, NtDllPatch.nDetourSize,
                       dwProtect, &dwProtect);
        ...
        if (VirtualProtect(NtDllPatch.pPatchAddress, NtDllPatch.nPatchSize,
          ...
          *(DWORD *)(&jmp[1]) = (DWORD)(NtDllPatch.pDetourAddress -
                                        (pReturnAddress - _countof(call)));
          memcpy(pReturnAddress - _countof(call) - _countof(jmp), &jmp,
                 _countof(jmp));

          // Call LdrpCallInitRoutine from eax/rax
          memcpy(pReturnAddress - _countof(call), &call, _countof(call));

          VirtualProtect(NtDllPatch.pPatchAddress, NtDllPatch.nPatchSize,
                         dwProtect, &dwProtect);
...
BOOL NtDllRestore(NTDLL_LDR_PATCH &NtDllPatch) {
  ...
  if (NtDllPatch.bState && NtDllPatch.nPatchSize && &NtDllPatch.pBackup[0]) {
    ...
    if (VirtualProtect(NtDllPatch.pPatchAddress, NtDllPatch.nPatchSize,
                       PAGE_EXECUTE_READWRITE, &dwProtect)) {
      memcpy(NtDllPatch.pPatchAddress, NtDllPatch.pBackup,
             NtDllPatch.nPatchSize);
      VirtualProtect(NtDllPatch.pPatchAddress, NtDllPatch.nPatchSize, dwProtect,
                     &dwProtect);
...
VisualLeakDetector::VisualLeakDetector() {
  ...
  m_options = 0x0;
  m_reportFile = NULL;
  wcsncpy_s(m_reportFilePath, MAX_PATH, VLD_DEFAULT_REPORT_FILE_NAME,
            _TRUNCATE);
  m_status = 0x0;
  ...
  if (ntdll) {
    ...
    RtlAllocateHeap =
        (RtlAllocateHeap_t)GetProcAddress(ntdll, "RtlAllocateHeap");
    RtlFreeHeap = (RtlFreeHeap_t)GetProcAddress(ntdll, "RtlFreeHeap");
    RtlReAllocateHeap =
        (RtlReAllocateHeap_t)GetProcAddress(ntdll, "RtlReAllocateHeap");

    LdrGetDllHandle =
        (LdrGetDllHandle_t)GetProcAddress(ntdll, "LdrGetDllHandle");
    LdrGetProcedureAddress = (LdrGetProcedureAddress_t)GetProcAddress(
  ...
  if (!IsWindows7OrGreater()) // kernel32.dll
  {
    if (kernel32)
      m_GetProcAddress =
          (GetProcAddress_t)GetProcAddress(kernel32, "GetProcAddress");
  } else {
    if (kernelBase) {
      m_GetProcAddress =
          (GetProcAddress_t)GetProcAddress(kernelBase, "GetProcAddress");
      m_GetProcAddressForCaller = (GetProcAddressForCaller_t)GetProcAddress(
          kernelBase, "GetProcAddressForCaller");
    }
  ...
  if (m_options & VLD_OPT_SELF_TEST) {
    ...
    if (m_options & VLD_OPT_UNICODE_REPORT) {
      wcsncpy_s(new WCHAR[wcslen(SELFTESTTEXTW) + 1], wcslen(SELFTESTTEXTW) + 1,
                SELFTESTTEXTW, _TRUNCATE);
      m_selfTestLine = __LINE__ - 1;
    } else {
      strncpy_s(new CHAR[strlen(SELFTESTTEXTA) + 1], strlen(SELFTESTTEXTA) + 1,
                SELFTESTTEXTA, _TRUNCATE);
      m_selfTestLine = __LINE__ - 1;
    }
  }
  ...
  if (m_tlsIndex == TLS_OUT_OF_INDEXES) {
    Report(L"ERROR: Visual Leak Detector could not be installed because thread "
           L"local"
  ...
  information on symbol searches, such as the directories it is searching and
  and error messages. In other words, this will really pollute the debug output
  window with extra messages. To enable this debug output to be displayed to the
  console without changing your source code, set the DBGHELP_DBGOUT environment
  variable to a non-NULL value before calling the SymInitialize function. To log
  the information to a file, set the DBGHELP_LOG environment variable to the
  name of the log file to be used.
  */
  g_DbgHelp.SymSetOptions(SYMOPT_DEBUG | SYMOPT_UNDNAME |
                          SYMOPT_DEFERRED_LOADS | SYMOPT_LOAD_LINES);
#else
  g_DbgHelp.SymSetOptions(SYMOPT_UNDNAME | SYMOPT_DEFERRED_LOADS |
                          SYMOPT_LOAD_LINES);
#endif
  DbgTrace(L"dbghelp32.dll %i: SymInitializeW\n", GetCurrentThreadId());
  if (!g_DbgHelp.SymInitializeW(g_currentProcess, symbolpath, FALSE)) {
    Report(L"WARNING: Visual Leak Detector: The symbol handler failed to "
           L"initialize (error=%lu).\n"
           L"    File and function names will probably not be available in "
           L"call stacks.\n",
           GetLastError());
  }
  delete[] symbolpath;
  ...
  DbgTrace(L"dbghelp32.dll %i: EnumerateLoadedModulesW64\n",
           GetCurrentThreadId());
  g_LoadedModules.EnumerateLoadedModulesW64(g_currentProcess, addLoadedModule,
                                            newmodules);
  ...
  if (m_status & VLD_STATUS_FORCE_REPORT_TO_FILE) {
    ...
    Report(L"NOTE: Visual Leak Detector: Unicode-encoded reporting has been "
           L"enabled, but the\n"
           L"  debugger is the only selected report destination. The debugger "
           L"cannot display\n"
           L"  Unicode characters, so the report will also be sent to a file. "
           L"If no file has\n"
           L"  been specified, the default file name is "
           L"\"" VLD_DEFAULT_REPORT_FILE_NAME L"\".\n");
  }
  reportConfig();
}
...
bool VisualLeakDetector::waitForAllVLDThreads() {
  ...
  for (TlsMap::Iterator tlsit = m_tlsMap->begin(); tlsit != m_tlsMap->end();
       ++tlsit) {
    ...
    HANDLE thread = OpenThread(SYNCHRONIZE | THREAD_QUERY_INFORMATION, FALSE,
                               (*tlsit).second->threadId);
    if (thread == NULL) {
...
void VisualLeakDetector::checkInternalMemoryLeaks() {
  ...
  while (header) {
    ...
    Report(L"ERROR: Visual Leak Detector: Detected a memory leak internal to "
           L"Visual Leak Detector!!\n");
    Report(
        L"---------- Block %Iu at " ADDRESSFORMAT L": %Iu bytes ----------\n",
    ...
    if (m_maxDataDump != 0) {
      Report(L"  Data:\n");
      if (m_options & VLD_OPT_UNICODE_REPORT) {
        DumpMemoryW(VLDBLOCKDATA(header), (m_maxDataDump < header->size)
                                              ? m_maxDataDump
                                              : header->size);
      } else {
        DumpMemoryA(VLDBLOCKDATA(header), (m_maxDataDump < header->size)
                                              ? m_maxDataDump
                                              : header->size);
      }
    }
  ...
  if (m_options & VLD_OPT_SELF_TEST) {
    if ((internalleaks == 1) && (strcmp(leakfile, m_selfTestFile) == 0) &&
        (leakline == m_selfTestLine)) {
      Report(L"Visual Leak Detector passed the memory leak self-test.\n");
    } else {
      Report(
          L"ERROR: Visual Leak Detector: Failed the memory leak self-test.\n");
    }
  }
}
...
VisualLeakDetector::~VisualLeakDetector() {
  ...
  if (m_status & VLD_STATUS_INSTALLED) {
    ...
    DbgTrace(L"dbghelp32.dll %i: EnumerateLoadedModulesW64\n",
             GetCurrentThreadId());
    g_LoadedModules.EnumerateLoadedModulesW64(g_currentProcess,
                                              detachFromModule, NULL);
    ...
    if (m_status & VLD_STATUS_NEVER_ENABLED) {
      ...
      Report(L"WARNING: Visual Leak Detector: Memory leak detection was never "
             L"enabled.\n");
    } else {
    ...
    if (!g_DbgHelp.SymCleanup(g_currentProcess)) {
      Report(L"WARNING: Visual Leak Detector: The symbol handler failed to "
             L"deallocate resources (error=%lu).\n",
             GetLastError());
    }

    {
      ...
      for (HeapMap::Iterator heapit = m_heapMap->begin();
           heapit != m_heapMap->end(); ++heapit) {
        BlockMap *blockmap = &(*heapit).second->blockMap;
        for (BlockMap::Iterator blockit = blockmap->begin();
             blockit != blockmap->end(); ++blockit) {
    ...
    if (threadsactive) {
      Report(L"WARNING: Visual Leak Detector: Some threads appear to have not "
             L"terminated normally.\n"
             L"  This could cause inaccurate leak detection results, including "
             L"false positives.\n");
    }
...
UINT32 VisualLeakDetector::getModuleState(ModuleSet::Iterator &it,
                                          UINT32 &moduleFlags) {
...

  {
    ...
    if (oldit != oldmodules->end()) // We've seen this "new" module loaded in
                                    // the process before.
  ...
  if (IsModulePatched((HMODULE)modulebase, m_patchTable,
                      _countof(m_patchTable))) {
...
VOID VisualLeakDetector::attachToLoadedModules(ModuleSet *newmodules) {
  ...
  for (ModuleSet::Iterator newit = newmodules->begin();
       newit != newmodules->end(); ++newit) {
    ...
    if ((state == 3) && (moduleFlags & VLD_MODULE_SYMBOLSLOADED)) {
      ...
      if (g_DbgHelp.SymUnloadModule64(g_currentProcess, modulebase, locker) ==
          false) {
        Report(L"WARNING: Visual Leak Detector: Failed to unload the symbols "
               L"for %s. Function names and line"
               L" numbers shown in the memory leak report for %s may be "
               L"inaccurate.\n",
    ...
    BOOL SymbolsLoaded = g_DbgHelp.SymGetModuleInfoW64(
        g_currentProcess, modulebase, &moduleimageinfo, locker);

    if (!SymbolsLoaded || moduleimageinfo.BaseOfImage != modulebase) {
      DbgTrace(L"dbghelp32.dll %i: SymLoadModuleEx\n", GetCurrentThreadId());
      DWORD64 module =
          g_DbgHelp.SymLoadModuleExW(g_currentProcess, NULL, modulepath, NULL,
                                     modulebase, modulesize, NULL, 0, locker);
      if (module == modulebase) {
        DbgTrace(L"dbghelp32.dll %i: SymGetModuleInfoW64\n",
                 GetCurrentThreadId());
        SymbolsLoaded = g_DbgHelp.SymGetModuleInfoW64(
            g_currentProcess, modulebase, &moduleimageinfo, locker);
      }
    }
    if (SymbolsLoaded)
    ...
    if (!GetModuleHandleEx(GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS,
                           (LPCTSTR)modulebase, &modulelocal))
      continue;

    if (!FindImport(modulelocal, m_vldBase, VLDDLL,
    ...
    if ((moduleFlags & VLD_MODULE_EXCLUDED) == 0 &&
            !(moduleFlags & VLD_MODULE_SYMBOLSLOADED) ||
        (moduleimageinfo.SymType == SymExport)) {
      ...
      // and function names.
      Report(L"WARNING: Visual Leak Detector: A module, %s, included in memory "
             L"leak detection\n"
             L"  does not have any debugging symbols available, or they could "
             L"not be located.\n"
             L"  Function names and/or line numbers for this module may not be "
             L"available.\n",
             modulename);
    }
...
LPWSTR VisualLeakDetector::buildSymbolSearchPath() {
  ...
  if (envlen != 0) {
    LPWSTR env = new WCHAR[envlen];
    if (GetEnvironmentVariable(L"_NT_ALTERNATE_SYMBOL_PATH", env, envlen) !=
        0) {
      path = AppendString(path, env);
      path = AppendString(path, L";");
    }
  ...
  for (UINT n = 9; n <= 14; ++n) {
    ...
    swprintf(debuggerpath, _countof(debuggerpath),
             L"Software\\Microsoft\\VisualStudio\\%u.0\\Debugger", n);
    HKEY debuggerkey;
    WCHAR symbolCacheDir[MAX_PATH] = {0};
    LSTATUS regstatus = RegOpenKeyExW(HKEY_CURRENT_USER, debuggerpath, 0,
                                      KEY_QUERY_VALUE, &debuggerkey);

    if (regstatus == ERROR_SUCCESS) {
      DWORD valuetype;
      DWORD dirLength = MAX_PATH * sizeof(WCHAR);
      regstatus =
          RegQueryValueExW(debuggerkey, L"SymbolCacheDir", NULL, &valuetype,
                           (LPBYTE)&symbolCacheDir, &dirLength);
      if (regstatus == ERROR_SUCCESS && valuetype == REG_SZ &&
          symbolCacheDir[0] != NULL && !wcsstr(path, symbolCacheDir)) {
...
BOOL VisualLeakDetector::GetIniFilePath(LPTSTR lpPath, SIZE_T cchPath) {
  ...
  if (0 < (written = GetCurrentDirectory(MAX_PATH, path))) {
    _tcsncpy_s(&path[written], MAX_PATH - written, TEXT("\\vld.ini"),
               _TRUNCATE);
    if (_tstat(path, &s) == 0) {
  ...
  if (0 < (written = GetModuleFileName(hModule, path, MAX_PATH))) {
    ...
    if (p &&
        0 == _tcsncpy_s(p, MAX_PATH - written, TEXT("\\vld.ini"), _TRUNCATE)) {
      if (_tstat(path, &s) == 0) {
        _tcsncpy_s(lpPath, cchPath, path, _TRUNCATE);
        return TRUE;
      }
  ...
  LONG regstatus = RegOpenKeyEx(HKEY_CURRENT_USER, VLDREGKEYPRODUCT, 0,
                                KEY_QUERY_VALUE, &productkey);
  if (regstatus == ERROR_SUCCESS) {
    length = MAX_PATH * sizeof(TCHAR);
    regstatus = RegQueryValueEx(productkey, TEXT("IniFile"), NULL, &valuetype,
                                (LPBYTE)&path, &length);
  ...
  // Get the location of the vld.ini file from the registry.
  regstatus = RegOpenKeyEx(HKEY_LOCAL_MACHINE, VLDREGKEYPRODUCT, 0,
                           KEY_QUERY_VALUE, &productkey);
  if (regstatus == ERROR_SUCCESS) {
    length = MAX_PATH * sizeof(TCHAR);
    regstatus = RegQueryValueEx(productkey, TEXT("IniFile"), NULL, &valuetype,
...
VOID VisualLeakDetector::configure() {
  ...
  if (!GetEnvironmentVariable(L"VLD", buffer, buffersize)) {
    GetPrivateProfileString(L"Options", L"VLD", L"on", buffer, buffersize,
                            inipath);
  }
  ...
  m_maxDataDump =
      LoadIntOption(L"MaxDataDump", VLD_DEFAULT_MAX_DATA_DUMP, inipath);
  m_maxTraceFrames =
      LoadIntOption(L"MaxTraceFrames", VLD_DEFAULT_MAX_TRACE_FRAMES, inipath);
  if (m_maxTraceFrames < 1) {
    m_maxTraceFrames = VLD_DEFAULT_MAX_TRACE_FRAMES;
  }

  // Read the force-include module list.
  LoadStringOption(L"ForceIncludeModules", m_forcedModuleList,
                   MAXMODULELISTLENGTH, inipath);
...
//
SIZE_T
VisualLeakDetector::eraseDuplicates(const BlockMap::Iterator &element,
  ...
  for (HeapMap::Iterator heapit = m_heapMap->begin();
    ...
    for (BlockMap::Iterator blockit = blockmap->begin();
      ...
      if (it != aggregatedLeaks.end())
        continue;
      if ((info->size == elementinfo->size) &&
          (*(info->callStack) == *(elementinfo->callStack))) {
...
VOID VisualLeakDetector::mapBlock(HANDLE heap, LPCVOID mem, SIZE_T size,
  ...
  if (blockit == blockmap->end()) {
    ...
    Report(L"VLD: New allocation at already allocated address: 0x%p with size: "
           L"%u and new size: %u\n",
           mem, info->size, size);
    delete info;
...
VOID VisualLeakDetector::unmapBlock(HANDLE heap, LPCVOID mem,
  ...
  if (blockit == blockmap->end()) {
    // This memory block is not in the block map. We must not have monitored
    // this allocation (probably happened before VLD was initialized).

    // This can also result from allocating on one heap, and freeing on another
    // heap. This is an especially bad way to corrupt the application. Now we
    // have to search through every heap and every single block in each to make
    // sure that this is indeed the case.
    if (m_options & VLD_OPT_VALIDATE_HEAPFREE) {
      HANDLE other_heap = NULL;
      blockinfo_t *alloc_block =
          findAllocedBlock(mem, other_heap); // other_heap is an out parameter
      bool diff =
          other_heap != heap; // Check indeed if the other heap is different
      if (alloc_block && alloc_block->callStack && diff) {
        Report(L"CRITICAL ERROR!: VLD reports that memory was allocated in one "
               L"heap and freed in another.\nThis will result in a corrupted "
               L"heap.\nAllocation Call stack.\n");
        Report(L"---------- Block %Iu "
               L"at " ADDRESSFORMAT L": %Iu bytes ----------\n",
...
//    None.
//
VOID VisualLeakDetector::remapBlock(HANDLE heap, LPCVOID mem, LPCVOID newmem,
                                    SIZE_T size, bool debugcrtalloc, bool ucrt,
                                    DWORD threadId, blockinfo_t *&pblockInfo,
                                    const context_t &context) {
  CriticalSectionLocker<> cs(g_heapMapLock);
...
VOID VisualLeakDetector::reportConfig() {
  ...
  if (m_forcedModuleList[0] != '\0') {
    Report(L"    Forcing %s of these modules in leak detection: %s\n",
           (m_options & VLD_OPT_MODULE_LIST_INCLUDE) ? L"inclusion"
                                                     : L"exclusion",
           m_forcedModuleList);
  }
...
bool VisualLeakDetector::isDebugCrtAlloc(LPCVOID block, blockinfo_t *info) {
  ...
  if (!info->debugCrtAlloc) {
    ...
    if (_BLOCK_TYPE_IS_VALID(crtheader->use) && nValid &&
        (nSize == info->size)) {
...
SIZE_T VisualLeakDetector::getLeaksCount(heapinfo_t *heapinfo, DWORD threadId) {
  ...
  for (BlockMap::Iterator blockit = blockmap->begin();
    ...
    if (isDebugCrtAlloc(block, info)) {
      ...
      // DynamicLoader/Thread test will randomly fail with less leaks being
      // reported.
...
SIZE_T VisualLeakDetector::reportHeapLeaks(HANDLE heap) {
  ...
  // Show a summary.
  if (leaks_count != 0) {
    Report(L"Visual Leak Detector detected %Iu memory leak%s in "
           L"heap " ADDRESSFORMAT L"\n",
...
SIZE_T VisualLeakDetector::reportLeaks(heapinfo_t *heapinfo, bool &firstLeak,
                                       Set<blockinfo_t *> &aggregatedLeaks,
                                       DWORD threadId) {
  BlockMap *blockmap = &heapinfo->blockMap;
  SIZE_T leaksFound = 0;
  ...
  for (BlockMap::Iterator blockit = blockmap->begin();
    ...
    if (info->callStack)
      callstackCRC =
          CalculateCRC32(info->size, info->callStack->getHashValue());
    Report(L"  Leak Hash: 0x%08X, Count: %Iu, Total %Iu bytes\n", callstackCRC,
           blockLeaksCount, size * blockLeaksCount);
...
// FindAllocedBlock - Find if a particular memory allocation is tracked inside
// of VLD.
//     This is a really good example of how to iterate through the data
//     structures that represent heaps and their associated memory blocks.
// Pre Condition: Be VERY sure that this is only called within a block that
// already has acquired a critical section for m_maplock.
//
...
//
blockinfo_t *VisualLeakDetector::findAllocedBlock(LPCVOID mem,
                                                  __out HANDLE &heap) {
  heap = NULL;
  blockinfo_t *result = NULL;
  // Iterate through all heaps
  CriticalSectionLocker<> cs(g_heapMapLock);
  for (HeapMap::Iterator it = m_heapMap->begin(); it != m_heapMap->end();
       ++it) {
...
//    enumerating.
//
BOOL VisualLeakDetector::addLoadedModule(PCWSTR modulepath, DWORD64 modulebase,
                                         ULONG modulesize, PVOID context) {
  ...
  _wsplitpath_s(modulepathw.c_str(), NULL, 0, NULL, 0, filename, _MAX_FNAME,
                extension, _MAX_EXT);
...
//    Always returns TRUE.
//
BOOL VisualLeakDetector::detachFromModule(PCWSTR /*modulepath*/,
                                          DWORD64 modulebase,
                                          ULONG /*modulesize*/,
                                          PVOID /*context*/) {
  UINT tablesize = _countof(m_patchTable);
...
FARPROC VisualLeakDetector::_GetProcAddress(HMODULE module, LPCSTR procname) {
  ...
  if (original) {
    ...
    for (UINT index = 0; index < tablesize; index++) {
      ...
      if ((entry->moduleBase == 0x0) ||
          ((HMODULE)entry->moduleBase != module)) {
...
//    the function, if there is a replacement function.
//
FARPROC VisualLeakDetector::_GetProcAddressForCaller(HMODULE module,
                                                     LPCSTR procname,
                                                     LPVOID caller) {
...
FARPROC VisualLeakDetector::_RGetProcAddressForCaller(HMODULE module,
                                                      LPCSTR procname,
                                                      LPVOID caller) {
...
//
NTSTATUS VisualLeakDetector::_LdrLoadDll(LPWSTR searchpath, PULONG flags,
                                         unicodestring_t *modulename,
...
NTSTATUS VisualLeakDetector::_LdrLoadDllWin8(DWORD_PTR reserved, PULONG flags,
                                             unicodestring_t *modulename,
                                             PHANDLE modulehandle) {
  // Load the DLL
...
NTSTATUS VisualLeakDetector::_LdrGetDllHandle(
    IN PWSTR DllPath OPTIONAL, IN PULONG DllCharacteristics OPTIONAL,
    IN PUNICODE_STRING DllName, OUT PVOID *DllHandle OPTIONAL) {
  LoaderLock ll;

  NTSTATUS status =
      LdrGetDllHandle(DllPath, DllCharacteristics, DllName, DllHandle);
  return status;
}
NTSTATUS VisualLeakDetector::_LdrGetProcedureAddress(
    IN PVOID BaseAddress, IN PANSI_STRING Name, IN ULONG Ordinal,
    OUT PVOID *ProcedureAddress) {
  LoaderLock ll;

  NTSTATUS status =
      LdrGetProcedureAddress(BaseAddress, Name, Ordinal, ProcedureAddress);
  return status;
}

NTSTATUS
VisualLeakDetector::_LdrLockLoaderLock(IN ULONG Flags,
...
NTSTATUS
VisualLeakDetector::_LdrUnlockLoaderLock(IN ULONG Flags,
...
VOID VisualLeakDetector::RefreshModules() {
  ...
  newmodules->reserve(MODULE_SET_RESERVE);
  {
    // Duplicate code here in this method. Consider refactoring out to another
    // method. Create a new set of all loaded modules, including any newly
    // loaded modules.
    DbgTrace(L"dbghelp32.dll %i: EnumerateLoadedModulesW64\n",
             GetCurrentThreadId());
    g_LoadedModules.EnumerateLoadedModulesW64(g_currentProcess, addLoadedModule,
...
CONST UINT32 OptionsMask =
    VLD_OPT_AGGREGATE_DUPLICATES | VLD_OPT_MODULE_LIST_INCLUDE |
    VLD_OPT_SAFE_STACK_WALK | VLD_OPT_SLOW_DEBUGGER_DUMP |
    VLD_OPT_START_DISABLED | VLD_OPT_TRACE_INTERNAL_FRAMES |
    VLD_OPT_SKIP_HEAPFREE_LEAKS | VLD_OPT_VALIDATE_HEAPFREE |
...
void VisualLeakDetector::SetOptions(UINT32 option_mask, SIZE_T maxDataDump,
                                    UINT32 maxTraceFrames) {
  if (m_options & VLD_OPT_VLDOFF) {
    // VLD has been turned off.
    return;
  }
...
void VisualLeakDetector::SetModulesList(CONST WCHAR *modules,
                                        BOOL includeModules) {
  if (m_options & VLD_OPT_VLDOFF) {
    // VLD has been turned off.
    return;
...
void VisualLeakDetector::SetReportOptions(UINT32 option_mask,
...
void VisualLeakDetector::setupReporting() {
  ...
  if (m_reportFile == NULL) {
    Report(L"WARNING: Visual Leak Detector: Couldn't open report file for "
           L"writing: %s\n"
...
blockinfo_t *VisualLeakDetector::getAllocationBlockInfo(void *alloc) {
  ...
  for (HeapMap::Iterator heapiter = m_heapMap->begin();
       heapiter != m_heapMap->end(); ++heapiter) {
...
const wchar_t *
VisualLeakDetector::GetAllocationResolveResults(void *alloc,
                                                BOOL showInternalFrames) {
  LoaderLock ll;
...
int VisualLeakDetector::resolveStacks(heapinfo_t *heapinfo) {
  ...
  for (BlockMap::Iterator blockit = blockmap.begin(); blockit != blockmap.end();
       ++blockit) {
    // Found a block which is still in the BlockMap. We've identified a
    // potential memory leak.
    ...
    if (info->callStack) {
      ...
      if ((m_options & VLD_OPT_SKIP_CRTSTARTUP_LEAKS) &&
          info->callStack->isCrtStartupAlloc()) {
        info->reported = true;
        continue;
      }
    }
...
CaptureContext::CaptureContext(void *func, context_t &context, BOOL debug,
                               BOOL ucrt)
    : m_context(context) {
...
CaptureContext::~CaptureContext() {
  ...
  if ((m_tls->blockWithoutGuard) && (!IsExcludedModule())) {
    ...
    if (m_tls->newBlockWithoutGuard == NULL) {
      g_vld.mapBlock(m_tls->heap, m_tls->blockWithoutGuard, m_tls->size,
                     (m_tls->flags & VLD_TLS_DEBUGCRTALLOC) != 0,
                     (m_tls->flags & VLD_TLS_UCRT) != 0, m_tls->threadId,
                     pblockInfo);
    } else {
      g_vld.remapBlock(m_tls->heap, m_tls->blockWithoutGuard,
                       m_tls->newBlockWithoutGuard, m_tls->size,
                       (m_tls->flags & VLD_TLS_DEBUGCRTALLOC) != 0,
                       (m_tls->flags & VLD_TLS_UCRT) != 0, m_tls->threadId,
...
void CaptureContext::Set(HANDLE heap, LPVOID mem, LPVOID newmem, SIZE_T size) {
  ...
  if ((m_tls->blockWithoutGuard) &&
      (g_vld.m_options & VLD_OPT_TRACE_INTERNAL_FRAMES)) {
    // If VLD_OPT_TRACE_INTERNAL_FRAMES is specified then we capture the frame
    // pointer upto the function that actually performs the allocation to the
    // heap being: HeapAlloc, HeapReAlloc, RtlAllocateHeap, RtlReAllocateHeap.
    m_tls->context = m_context;
  }
}
