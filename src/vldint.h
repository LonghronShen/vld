...
typedef void *(__cdecl *_recalloc_dbg_t)(void *, size_t, size_t, int,
                                         const char *, int);
...
typedef wchar_t *(__cdecl *_wcsdup_dbg_t)(const wchar_t *, int, const char *,
                                          int);
...
typedef void *(__cdecl *_aligned_offset_realloc_t)(void *, size_t, size_t,
                                                   size_t);
typedef void *(__cdecl *_aligned_recalloc_t)(void *, size_t, size_t, size_t);
typedef void *(__cdecl *_aligned_offset_recalloc_t)(void *, size_t, size_t,
                                                    size_t, size_t);
typedef void *(__cdecl *_aligned_malloc_dbg_t)(size_t, size_t, int,
                                               const char *, int);
typedef void *(__cdecl *_aligned_offset_malloc_dbg_t)(size_t, size_t, size_t,
                                                      int, const char *, int);
typedef void *(__cdecl *_aligned_realloc_dbg_t)(void *, size_t, size_t, int,
                                                const char *, int);
typedef void *(__cdecl *_aligned_offset_realloc_dbg_t)(void *, size_t, size_t,
                                                       size_t, int,
                                                       const char *, int);
typedef void *(__cdecl *_aligned_recalloc_dbg_t)(void *, size_t, size_t, size_t,
                                                 int, const char *, int);
typedef void *(__cdecl *_aligned_offset_recalloc_dbg_t)(void *, size_t, size_t,
                                                        size_t, size_t, int,
                                                        const char *, int);
...
struct moduleinfo_t {
  ...
  SIZE_T addrLow;  // Lowest address within the module's virtual address space
                   // (i.e. base address).
  SIZE_T addrHigh; // Highest address within the module's virtual address space
                   // (i.e. base + size).
  UINT32 flags;    // Module flags:
#define VLD_MODULE_EXCLUDED                                                    \
  0x1 //   If set, this module is excluded from leak detection.
#define VLD_MODULE_SYMBOLSLOADED                                               \
  0x2             //   If set, this module's debug symbols have been loaded.
...
struct tls_t {
  context_t context; // Address of return address at the first call that entered
                     // VLD's code for the current allocation.
  UINT32 flags;      // Thread-local status flags:
#define VLD_TLS_DEBUGCRTALLOC                                                  \
  0x1 //   If set, the current allocation is a CRT allocation.
#define VLD_TLS_DISABLED                                                       \
  0x2 //   If set, memory leak detection is disabled for the current thread.
#define VLD_TLS_ENABLED                                                        \
  0x4 //   If set, memory leak detection is enabled for the current thread.
#define VLD_TLS_UCRT                                                           \
  0x8              //   If set, the current allocation is a UCRT allocation.
...
// 2. HeapAlloc set tls->heap, tls->blockWithoutGuard, tls->newBlockWithoutGuard
// and tls->size
// 3. Allocation function reset tls data, map block and capture callstack to
// tls->blockWithoutGuard
...
public:
  CaptureContext(void *func, context_t &context, BOOL debug = FALSE,
                 BOOL ucrt = FALSE);
  ~CaptureContext();
...
public:
  ...
  VOID SetOptions(UINT32 option_mask, SIZE_T maxDataDump,
  ...
  const wchar_t *GetAllocationResolveResults(void *alloc,
                                             BOOL showInternalFrames);

  static NTSTATUS __stdcall _LdrLoadDll(LPWSTR searchpath, PULONG flags,
                                        unicodestring_t *modulename,
                                        PHANDLE modulehandle);
  static NTSTATUS __stdcall _LdrLoadDllWin8(DWORD_PTR reserved, PULONG flags,
                                            unicodestring_t *modulename,
                                            PHANDLE modulehandle);
  static FARPROC __stdcall _RGetProcAddress(HMODULE module, LPCSTR procname);
  static FARPROC __stdcall _RGetProcAddressForCaller(HMODULE module,
                                                     LPCSTR procname,
                                                     LPVOID caller);

  static NTSTATUS NTAPI _LdrGetDllHandle(IN PWSTR DllPath OPTIONAL,
                                         IN PULONG DllCharacteristics OPTIONAL,
                                         IN PUNICODE_STRING DllName,
                                         OUT PVOID *DllHandle OPTIONAL);
  static NTSTATUS NTAPI _LdrGetProcedureAddress(IN PVOID BaseAddress,
                                                IN PANSI_STRING Name,
                                                IN ULONG Ordinal,
                                                OUT PVOID *ProcedureAddress);
  static NTSTATUS NTAPI _LdrUnloadDll(IN PVOID BaseAddress);
  static NTSTATUS NTAPI _LdrLockLoaderLock(IN ULONG Flags,
                                           OUT PULONG Disposition OPTIONAL,
                                           OUT PULONG_PTR Cookie OPTIONAL);
  static NTSTATUS NTAPI _LdrUnlockLoaderLock(IN ULONG Flags,
                                             IN ULONG_PTR Cookie OPTIONAL);

private:
  ////////////////////////////////////////////////////////////////////////////////
  // Private leak detection functions - see each function definition for
  // details.
  ////////////////////////////////////////////////////////////////////////////////
  ...
  SIZE_T eraseDuplicates(const BlockMap::Iterator &element,
                         Set<blockinfo_t *> &aggregatedLeak);
  tls_t *getTls();
  VOID mapBlock(HANDLE heap, LPCVOID mem, SIZE_T size, bool crtalloc, bool ucrt,
  ...
  VOID remapBlock(HANDLE heap, LPCVOID mem, LPCVOID newmem, SIZE_T size,
                  bool crtalloc, bool ucrt, DWORD threadId,
  ...
  SIZE_T reportLeaks(heapinfo_t *heapinfo, bool &firstLeak,
  ...
  static BOOL __stdcall addLoadedModule(PCWSTR modulepath, DWORD64 modulebase,
                                        ULONG modulesize, PVOID context);
  static BOOL __stdcall detachFromModule(PCWSTR modulepath, DWORD64 modulebase,
                                         ULONG modulesize, PVOID context);
  ...
  static FARPROC __stdcall _GetProcAddressForCaller(HMODULE module,
                                                    LPCSTR procname,
                                                    LPVOID caller);

  static HANDLE __stdcall _GetProcessHeap();

  static HANDLE __stdcall _HeapCreate(DWORD options, SIZE_T initsize,
                                      SIZE_T maxsize);
  ...
  static LPVOID __stdcall _HeapReAlloc(HANDLE heap, DWORD flags, LPVOID mem,
                                       SIZE_T size);

  static LPVOID __stdcall _RtlAllocateHeap(HANDLE heap, DWORD flags,
                                           SIZE_T size);
  static BYTE __stdcall _RtlFreeHeap(HANDLE heap, DWORD flags, LPVOID mem);
  static LPVOID __stdcall _RtlReAllocateHeap(HANDLE heap, DWORD flags,
                                             LPVOID mem, SIZE_T size);
  ...
  // Private data
  ////////////////////////////////////////////////////////////////////////////////
  WCHAR m_forcedModuleList[MAXMODULELISTLENGTH]; // List of modules to be
                                                 // forcefully included in leak
                                                 // detection.
  ...
  ModuleSet *m_loadedModules; // Contains information about all modules loaded
                              // in the process.
  SIZE_T m_maxDataDump;    // Maximum number of user-data bytes to dump for each
                           // leaked block.
  UINT32 m_maxTraceFrames; // Maximum number of frames per stack trace for each
                           // leaked block.
  CriticalSection
      m_modulesLock; // Protects accesses to the "loaded modules" ModuleSet.
  CriticalSection
      m_optionsLock; // Serializes access to the heap and block maps.
  ...
  static moduleentry_t m_patchTable[58]; // Table of imports patched for
                                         // attaching VLD to other modules.
  FILE *m_reportFile; // File where the memory leak report may be sent to.
  WCHAR m_reportFilePath[MAX_PATH]; // Full path and name of file to send memory
                                    // leak report to.
  const char *m_selfTestFile; // Filename where the memory leak self-test block
                              // is leaked.
  int m_selfTestLine; // Line number where the memory leak self-test block is
                      // leaked.
  UINT32 m_status;    // Status flags:
#define VLD_STATUS_DBGHELPLINKED                                               \
  0x1 //   If set, the explicit dynamic link to the Debug Help Library
      //   succeeded.
#define VLD_STATUS_INSTALLED 0x2 //   If set, VLD was successfully installed.
#define VLD_STATUS_NEVER_ENABLED                                               \
  0x4 //   If set, VLD started disabled, and has not yet been manually enabled.
#define VLD_STATUS_FORCE_REPORT_TO_FILE                                        \
  0x8               //   If set, the leak report is being forced to a file.
  DWORD m_tlsIndex; // Thread-local storage index.
  CriticalSection m_tlsLock; // Protects accesses to the Set of TLS structures.
  TlsMap
      *m_tlsMap; // Set of all thread-local storage structures for the process.
